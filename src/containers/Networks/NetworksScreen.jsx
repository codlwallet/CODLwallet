                            ...
                            <TouchableOpacity style={[styles.buttonContainer, { backgroundColor: btnIndex[item?.value] ? colors.white : colors.gray }]} key={index} onPress={() => setBtnIndex({ ...btnIndex, [item?.value]: !btnIndex[item?.value] })}>
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    {item.value === appConstant.zksync ?
                                        <Image source={item.image} style={{ height: hp(3), width: hp(5), backgroundColor: 'transparent', tintColor: btnIndex[item?.value] ? '#495057' : colors.white }} /> :
                                        item.value === appConstant.ethereum ?
                                            ...
                                            item.value === appConstant.solana ?
                                                ...
                                                item.value === appConstant.avalanche ?
                                                    <Image source={item.image} style={{ height: hp(3.5), width: hp(4), right: wp(1), tintColor: btnIndex[item?.value] ? '#495057' : colors.white }} />
                                                    :
                                                    item.value === appConstant.bsc ?
                                                        <Image source={item.image} style={{ height: hp(4), width: hp(4), tintColor: btnIndex[item?.value] ? '#495057' : colors.white }} />
                                                        :
                                                        item.value === appConstant.optimism ?
                                                            <Image source={item.image} style={{ height: hp(5), width: hp(5.2), tintColor: btnIndex[item?.value] ? '#495057' : colors.white }} />
                                                            :
